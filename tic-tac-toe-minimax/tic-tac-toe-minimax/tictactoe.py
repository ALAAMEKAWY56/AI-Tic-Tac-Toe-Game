...
def minimax(board):
    ...
    # Calculate maximum depth of the search tree
    max_depth = len(actions(board))

    # Calculate branching factor
    branching_factor = calculate_branching_factor(board)

    # Calculate space complexity
    space_complexity = branching_factor * max_depth

    print(f"Time Complexity: {time_taken:.2f} nanoseconds")
    print(f"Space Complexity: O(bm) = {branching_factor} * {max_depth} = {space_complexity}")

    return best_move


def calculate_branching_factor(board):
    """
    Returns the average branching factor of the search tree.
    """
    total_branches = 0
    total_nodes = 0

    for depth in range(1, len(actions(board)) + 1):
        total_branches += len(actions(board)) ** depth
        total_nodes += len(actions(board)) ** (depth - 1)
